...
/**Illustrating Arrow Functions ('function' keyword replaced with '=>' */
var logger = output => {
    console.log(output);
}
...
/**Illustrationg const and let. /**Illustrating Scoping.
 * In place of var for variable declaration, use const or let.
 * Const is for variables whose assigned values will not be changed (immutable).
 * Let is for variables whose assigned values can change (mutable)
 */
...
export const genericFunction = () => {
    ...
    const [firstLang, secondLang, ...otherLangs] = Languages; //assign first element in languages array to firstLang
    return `First language is ${firstLang} and the second is ${secondLang} and the rest are ${otherLangs}`
}

/**PAssing PARAMETER TO FUNCTION
 * Rest - Uses Spread Operator
 */
...
export const multiplier2 = (...numbers) => {
    ...
    //loop through using the old for Each approach which still works
